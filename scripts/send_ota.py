...
def on_message(client, userdata, msg):
    ...
    match = []
    if regex(r'/([0-9a-zA-Z]+)/gatewayOta/erase$', msg.topic, match):
        q.put(["erase", match[0]])
    elif regex(r'([0-9a-zA-Z]+)/gatewayOta/md5/([0-9a-zA-Z]+)', msg.topic, match):
        q.put(["md5", match[0], match[1], msg.payload])
    elif regex(r'([0-9a-zA-Z]+)/gatewayOta/check$', msg.topic, match):
...
def main():
    ...
    parser.add_argument("--mqtt_id", help=("Specific Client key"))
    ...
    if args.id:
        outTopic = outTopic + "/" + args.id



    print("OUTTOPIC: " + outTopic)

    if args.id:
        send_topic = "{}/{}/gatewayOta/".format(inTopic, args.id)
    elif args.node:
        send_topic = "{}/{}/gatewayOta/".format(inTopic, args.node)
    else:
    ...
    # client = mqtt.Client(client_id="Server3")
